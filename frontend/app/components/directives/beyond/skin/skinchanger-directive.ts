...
export var directiveName = 'skinChanger';

class SkinChangerDirective implements angular.IDirective {

  restrict = 'AC';

  static $inject = ['$rootScope', '$filter', '$state', '$stateParams'];

  constructor(private $rootScope, private $filter, private $state, private $stateParams) {}
    ...
    el.on('click', () => {
      setTimeout(() => {
        this.$rootScope.$apply(() => {
          var skincolor = this.$filter('filter')(scope.skins, function (d) { return d.skin === el.attr('rel'); })[0];
          this.$rootScope.settings.skin = skincolor.skin;
          this.$rootScope.settings.color = skincolor.color;
...
export var Directive = [ '$injector', ($injector: angular.auto.IInjectorService) => {
  return $injector.instantiate(SkinChangerDirective);
}];