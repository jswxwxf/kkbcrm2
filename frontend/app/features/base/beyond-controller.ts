...
export var controllerName = 'base.BeyondController';

class BeyondController {

  static $inject = ['$rootScope', '$localStorage', '$state', '$timeout'];

  constructor(private $rootScope, private $localStorage, private $state, private $timeout) {
    ...
    $rootScope.$watch('settings.rtl', function () {
      if ($state.current.name != "persian.dashboard" && $state.current.name != "arabic.dashboard") {
        window['switchClasses']("pull-right", "pull-left");
        window['switchClasses']("databox-right", "databox-left");
        window['switchClasses']("item-right", "item-left");
      }
...
export class Controller extends BeyondController {}