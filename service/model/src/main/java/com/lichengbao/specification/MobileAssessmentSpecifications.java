...
class MobileAssessmentSpecDef extends SpecDef {
 ...
 */
@Component
public class MobileAssessmentSpecifications extends AbstractSpecifications {
	
	public Specification<MobileAssessment> isOwner(String name) {
		return (root, query, cb) -> {
			Join<MobileAssessment, User> owner = root.join("user");
			return cb.or(
	...
	SpecDef getSpecDef(SearchParameter search) {
		try {
			return mapper.readValue(search.getValue(), MobileAssessmentSpecDef.class);
		} catch (IOException e) {
