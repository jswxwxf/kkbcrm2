...
class UserCardApplySpecDef extends SpecDef {
 ...
 */
@Component
public class UserCardApplySpecifications extends AbstractSpecifications {
	
	public Specification<UserCardApply> isOwner(String name) {
		return (root, query, cb) -> {
			Join<UserCardApply, User> owner = root.join("user");
			return cb.or(
	...
	SpecDef getSpecDef(SearchParameter search) {
		try {
			return mapper.readValue(search.getValue(), UserCardApplySpecDef.class);
		} catch (IOException e) {
	...
	List<Specification> getSpecifications(SpecDef def) {
		...
		UserCardApplySpecDef specDef = (UserCardApplySpecDef) def;
