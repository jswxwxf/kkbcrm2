...
class UserBalanceSpecDef extends SpecDef {
 ...
 */
@Component
public class UserBalanceSpecifications extends AbstractSpecifications {
	
	public Specification<UserBalance> isOwner(String name) {
		return (root, query, cb) -> {
			Join<UserBalance, User> owner = root.join("user");
			return cb.or(
	...
	SpecDef getSpecDef(SearchParameter search) {
		try {
			return mapper.readValue(search.getValue(), UserBalanceSpecDef.class);
		} catch (IOException e) {
	...
	List<Specification> getSpecifications(SpecDef def) {
		...
		UserBalanceSpecDef specDef = (UserBalanceSpecDef) def;
