...
class UserCardSpecDef extends SpecDef {
 ...
 */
@Component
public class UserCardSpecifications extends AbstractSpecifications {
	
	public Specification<UserCard> isOwner(String name) {
	...
	SpecDef getSpecDef(SearchParameter search) {
		try {
			return mapper.readValue(search.getValue(), UserCardSpecDef.class);
		} catch (IOException e) {
	...
	List<Specification> getSpecifications(SpecDef def) {
		...
		UserCardSpecDef specDef = (UserCardSpecDef) def;
